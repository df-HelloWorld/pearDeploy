...
var returnFieldTypeData;// 字段类型的下拉框值
var account = {
    //地址
    url:{
        list_url : ctx + '/returntemplate/list.do',
        dataList_url : ctx + "/returntemplate/dataList.do",
        add_url : ctx+ "/returntemplate/add.do",
        update_url : ctx+ "/returntemplate/update.do",
        queryId_url: ctx+ "/returntemplate/getId.do",
        delete_url: ctx+ "/returntemplate/delete.do",
        manyOperation_url: ctx+ "/returntemplate/manyOperation.do",
        update_return_field_url: ctx+ "/returntemplate/updateReturnField.do",
        delete_field_url: ctx+ "/returnfield/delete.do"
    },
    ...
    list:[
        ...
        {"data":"id",
            "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                var html="";
                // html += '<a id = "returnField" directkey="' + oData.id + '" directValue="' + oData.gewayCodeName + '" href = "javascript:void(0);"> '+oData.gewayCodeName+ '</a>'
                html += '<input type="radio" name="returnTemplateId" id="returnTemplateId" directkey="' + oData.id + '" value="' + oData.id + '">';
                $(nTd).html(html);
            }
        },
        ...
        {"data":"id",
            "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                var html = '';
                html += '<a class = "dataTableBtn dataTableResetBtn " href="'+ctx+'/returntemplate/jumpUpdate.do?id='+oData.id+'"> 编辑 </a>';
                if (oData.isEnable == 1){
    ...
    indexInit : function (){
        ...
        this.queryReturnFieldType();
        this.queryGewayCodeAll();
        //添加
        $(".addbtn").live("click",function(){
            // var id = $("#returnTemplateId").val();
            var id = $("input[name='returnTemplateId']:checked").val();
            if (id == undefined){
                window.location.href = ctx + "/returntemplate/jumpAdd.do?id=0";
            } else{
                window.location.href = ctx + "/returntemplate/jumpAdd.do?id=" + id;
            }
        });
        ...
        //返回字段
        $("#returnField").live("click",function(){
            var id = $(this).attr('directkey');
            var gewayCodeName = $(this).attr('directValue');
            $("#divGewayCodeName").html(gewayCodeName);

            $.ajax({url : ctx+ "/returntemplate/getFieldById.do",
                type : 'post',
                dataType : 'json',
                data :{
                    id:id
    ...
    //下拉框数据填充
    //查询所有返回字段的字段类型-无分页-下拉框选项:
    queryReturnFieldType:function(){
...
function addTr(){
    ...
    for (var j=0;j<returnFieldTypeData.length>0;j++) {
        shtml_fieldType +="<option value="+returnFieldTypeData[j].stgValueTwo+">"+returnFieldTypeData[j].stgValueOne+"</option>";
    }
...
function submitHandler() {
    var url = account.url.update_return_field_url;
...
/**
 * 在复制新增的时候，页面初始化加载返回字段数据
 */
function loadReturnField(){
    var returntemplateId = $("#mbId").val();
    $.ajax({url : ctx+ "/returntemplate/getFieldById.do",
        type : 'post',
        dataType : 'json',
        data :{
...
/**
 * 在编辑的时候，页面初始化加载返回字段数据
 */
function loadReturnFieldUpdate(){
    var returntemplateId = $("#mbId").val();
    $.ajax({url : ctx+ "/returntemplate/getFieldById.do",
        type : 'post',
        dataType : 'json',
